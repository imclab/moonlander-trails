...
function Landscape(){
	...
	var render = this.render = function(polyline, view) { 
		...
		// we can assume the polyline has at least one point. 
		var points = polyline.points; 
		var line = lines[i];
		//c.moveTo(line.p1.x + offset, line.p1.y);  
		var pointcount = 0; 
		var p = points.getItem(pointcount); 
		p.x = line.p1.x + offset; 
		p.y = line.p1.y;
		...
		while((line = lines[i]).p1.x+offset<view.right) { 
			...
			var point = line.p2;
			while(pointcount>=points.numberOfItems) {
				points.appendItem(svg.createSVGPoint()); 
				//console.log("adding point "+points.numberOfItems); 
			}
			var p = points.getItem(pointcount++);
			p.x = point.x+offset;
			p.y = point.y; 
		...
		while(pointcount<points.numberOfItems) {
			//console.log("removing point" + points.numberOfItems); 
			points.removeItem(pointcount); 
		}
