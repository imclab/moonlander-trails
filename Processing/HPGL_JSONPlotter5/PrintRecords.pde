...
int addPrintRecord(long minTime, long maxTime, float x, float y, float scale) { 
  ...
  row.setString("editionCode", getEditionCodeForNumber(lastIndex)); 
...
String getEditionCodeForNumber(int index) { 

  String returncode = ""+char((index%26)+65);
  returncode = char((floor(index/26)%26)+65) +returncode; 
  println(returncode); 
  return returncode; 
}
