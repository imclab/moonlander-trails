...
PVector dataOffset = new PVector(376, 470); 
float dataScale = 0.588; 
...
Date startTime = new Date("12/01/2012");  // MM/DD/YY
...
void draw() {
  ...
  dirty = dirty | sliderLeft.update(); 
  dirty = dirty | sliderRight.update();
...
void renderTimes(boolean sendToPlotter) { 
  ...
  int milsPerHour = 3600000;
  int milsPer30Mins = milsPerHour/2;
  int divisions = 300000 / 5 *2; // every 2 mins 

  float ypos = topMargin - 30; 
  long offset = (divisions - (minTime%divisions))%divisions;

  for (long t = offset; t<=maxTime-minTime + divisions; t+=divisions) { 

    //if ( (t%60000 ==0) && (t%1000 ==0)) {
    float xpos = map(t, 0, maxTime-minTime, leftMargin, rightMargin); 
    if ((minTime + t)%(milsPerHour) == 0) {
      if (sendToPlotter) hpglManager.line(xpos, ypos-1, xpos, ypos-6); else line(xpos, ypos-1, xpos, ypos-6);

      float writingSize =  0.7; 
      plotText(getHourAndMinuteString(minTime+t), xpos - (6*2*writingSize), ypos - 10, writingSize, sendToPlotter);
    } else if ((minTime + t)%(milsPer30Mins) == 0) {

      if (sendToPlotter) hpglManager.line(xpos, ypos-1, xpos, ypos-4); else line(xpos, ypos-1, xpos, ypos-4);
    } else if (((minTime + t)%divisions) == 0) { 

      if (sendToPlotter) hpglManager.line(xpos, ypos-1, xpos, ypos-2); else line(xpos, ypos-1, xpos, ypos-2);
    }
  }
  ...
  for (int i = 0; i<paths.size(); i++) { 
    ...
    float lineHeight = clamp(map(numpoints, 0, 2000, 0, 20), 0, 20);

    if (sendToPlotter) hpglManager.line(xpos, ypos+1, xpos, ypos+1+lineHeight); else line(xpos, ypos+1, xpos, ypos+1+lineHeight);
  }
}
...
void plotCross(float x, float y, float size) { 
  hpglManager.line(x, y, x, y+size); 
  hpglManager.line(x, y, x, y-size); 
  hpglManager.line(x, y, x+size, y);
  hpglManager.line(x, y, x-size, y);
